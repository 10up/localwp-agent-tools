 ...
 */
export function getLightningServicesPath(): string {
 ...
 */
export function getRunPath(siteId: string): string {
 ...
 */
export async function findPhpBinary(phpVersion: string): Promise<string | null> {
	const serviceDir = await findServiceDir('php', phpVersion);
	...
	for (const platformDir of platformDirs) {
		...
		if (await fs.pathExists(altPhpBin)) {
			return altPhpBin;
		}
	}
 ...
 */
export async function findMysqlBinary(mysqlVersion: string, serviceName: string = 'mysql'): Promise<string | null> {
	const serviceDir = await findServiceDir(serviceName, mysqlVersion);
	if (!serviceDir) {
	...
	for (const platformDir of platformDirs) {
		...
		if (await fs.pathExists(mysqlBin)) {
			return mysqlBin;
		}
	}
 ...
 */
export async function findWpCli(phpVersion?: string): Promise<string | null> {
	...
	if (await fs.pathExists(userDataWpCli)) {
 ...
 */
function getSystemWpCliLocations(): string[] {
 ...
 */
function getLocalAppResourcesCandidates(): string[] {
	const home = os.homedir();
 ...
 */
export function getLocalAppResourcesPath(): string | null {
