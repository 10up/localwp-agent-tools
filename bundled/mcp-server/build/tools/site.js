...
async function handleGetSiteInfo() {
    ...
    if (wpCliBin && existsSync(wpCliBin)) {
        try {
            const { stdout: pluginOut } = await execFileAsync(PHP_BIN, [wpCliBin, 'plugin', 'list', '--status=active', '--format=json', `--path=${WP_PATH}`, '--skip-themes', '--skip-plugins'], { cwd: WP_PATH, timeout: 15_000, env: { ...process.env, PHP: PHP_BIN } });
        ...
        try {
            const { stdout: themeOut } = await execFileAsync(PHP_BIN, [wpCliBin, 'theme', 'list', '--status=active', '--format=json', `--path=${WP_PATH}`, '--skip-themes', '--skip-plugins'], { cwd: WP_PATH, timeout: 15_000, env: { ...process.env, PHP: PHP_BIN } });
...
// ── site_health_check ──────────────────────────────────────────────────
async function handleSiteHealthCheck() {
    const checks = [];
    ...
    if (wpCliBin && existsSync(wpCliBin)) {
        try {
            ...
            if (DB_SOCKET)
                wpCliArgs.splice(3, 0, '-d', `mysqli.default_socket=${DB_SOCKET}`, '-d', `pdo_mysql.default_socket=${DB_SOCKET}`);
            ...
            if (DB_SOCKET)
                countArgs.splice(3, 0, '-d', `mysqli.default_socket=${DB_SOCKET}`, '-d', `pdo_mysql.default_socket=${DB_SOCKET}`);
