 ...
 */
function callBridge(action: string, siteId?: string): Promise<BridgeResponse> {
  return new Promise((resolve, reject) => {
    const socketPath = BRIDGE_SOCKET;
    if (!socketPath) {
      reject(
        new Error(
          'BRIDGE_SOCKET environment variable is not configured. ' +
    ...
    client.on('end', () => {
      ...
      if (!trimmed) return; // Already resolved via 'data' handler
      try {
    ...
    client.on('error', (err: NodeJS.ErrnoException) => {
      if (err.code === 'ENOENT' || err.code === 'ECONNREFUSED') {
        reject(
          new Error(
            'Cannot connect to the Local bridge server. ' +
 ...
 */
function resolveSiteId(siteId?: string): string | undefined {
    ...
    inputSchema: {
      ...
      properties: {
        siteId: {
          type: 'string',
          description:
            'The Local site ID. Optional — defaults to the current site.',
        },
      },
    },
  },
...
async function handleSiteAction(
  ...
  const siteId = resolveSiteId(args.siteId as string | undefined);
      ...
      content: [
        {
          type: 'text',
          text:
            'Error: No siteId provided and SITE_ID environment variable is not set. ' +
...
// ── list_sites ────────────────────────────────────────────────────────
async function handleListSites(): Promise<{
